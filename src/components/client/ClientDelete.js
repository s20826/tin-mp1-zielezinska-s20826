...
import {getClientByIdApiCall, deleteClientApiCall} from '../../apiCalls/clientApiCalls';
...
function ClientDelete(){
    ...
    const client = getClientByIdApiCall(idClient)
    const {t} = useTranslation();


    const deleteClient = () => {
        deleteClientApiCall(idClient)
    ...
    return(
        <div className="container">
            <main>
                <div>
                    <h1>{t('client.pages.delete')}</h1>
                    <input type="hidden" name="animalId" value={client.idClient}/>

                    <Link className="form-actions-button-submit"  to={`../../clients`} onClick={() => deleteClient()} >
                    ...
                    <Link className="form-button-cancel"  to={`../../clients`} >
